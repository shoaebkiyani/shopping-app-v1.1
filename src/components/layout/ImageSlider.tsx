		...
		<>
			<div className='xs:flex-wrap flex sm:h-[280px] md:h-[350px] lg:h-[550px]'>
				<div
					...
					className='w-full xs:h-[280px] lg:h-[550px] relative group'
				>
					<div
						style={{ backgroundImage: `url(${slides[currentIndex].url})` }}
						className='h-full bg-cover bg-no-repeat transition-all duration-500'
					></div>
					...
					<div className='flex top-4 justify-center py-2 my-[-50px] text-zinc-200'>
						{slides.map((_slide, slideIndex) => (
							<div key={slideIndex} className='text-2xl cursor-pointer'>
								<RxDotFilled onClick={() => dotSlide(slideIndex)} />
							</div>
						))}
					</div>
				</div>
